...
export async function useTicketInfo(tenantId: number, eventId: number, ticketId: number) {
    const { data } = await useApi(`/api/tenant/${tenantId}/event/${eventId}/ticket/${ticketId}`, {
        transform: res => res.data,
    })
